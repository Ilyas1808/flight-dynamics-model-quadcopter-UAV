...
class Quadcopter():
    ...
    # Functie voor Euler van body-frame tot globale inertial frame
    ...
    def find_lin_acc(self,wind):
        ...
        wind_force_x = 0.5*self.A_ref*self.density*(math.pow(wind[0],2))*np.sign(wind[0]) #math.cos(self.angle[0]
        wind_force_y = 0.5*self.A_ref*self.density*(math.pow(wind[1],2))*np.sign(wind[1]) #math.sin(self.angle[1]
        wind_force = np.array([wind_force_x,wind_force_y,0]) #zonder np.array
    ...
    def des2speeds(self,thrust_des, tau_des):
        e1 = tau_des[0] * self.Ixx #Koppel in x-richting * traagheidsmoment Ixx
